...
async def callback_handler(message: types.Message, callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if action == settings.ACTIONS.PRODUCTS:
        ...
        await bot_functions.create_product(bot=bot, 
                                        user_id=user_id, 
                                        state=state, 
                                        message_id=callback_query.message.message_id)

    elif await state.get_data() == {}:
        await callback_query.message.answer(await state.get_data())

    elif message.text:
        await message.answer(await state.get_data())

...
for i in range(circle_count):
    circles.append({
        ...
        "velocity_x": randint(-10, 10), 
        "velocity_y": randint(-10, 10),
        "circle_radius": circle_radius
    })

# Функция для отрисовки круга
def draw_circle(x, y, velosity_x, velosity_y):
    pygame.draw.circle(screen, (255, 255, 255), (x, y), circle_radius)
    pygame.draw.line(screen, (255, 0, 0), (x, y), ((x+velosity_x), (y+velosity_y)), width=6)
...
while running:
    ...
    for i in range(len(circles)):
        ...
        draw_circle(circles[i]["circle_x"], circles[i]["circle_y"], 
                    circles[i]["velocity_x"]*3, 
                    circles[i]["velocity_y"]*3)
